...
final class MB_Prune {
	...
	public static function run(): array {
		...
		if ( ! empty( $ids ) ) {
			...
			$report['ticket_msgs'] = (int) $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . MB_DB::t( 'ticket_messages' ) . " WHERE ticket_id IN ({$placeholders})", $ids ) );
			$report['tickets']     = (int) $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . MB_DB::t( 'tickets' ) . " WHERE id IN ({$placeholders})", $ids ) );
		}
	 ...
	 *
	 * نسخهٔ قبل چیز دیگری می‌کرد: هر نتیجهٔ قدیمی‌تر از دو سال را پاک می‌کرد (حتی
	 * اگر جزو ۲۰ نتیجهٔ آخر آن کاربر بود) و در عوض تعداد نامحدودی نتیجهٔ دو سال
	 * اخیر را نگه می‌داشت. این پیاده‌سازی همان قاعدهٔ مستندشده است.
	 */
	private static function trim_quiz_results(): int {
		...
		$rows = (array) $wpdb->get_results(
			"SELECT id, user_id, quiz_id FROM {$table} ORDER BY user_id ASC, quiz_id ASC, created_at DESC, id DESC",
			ARRAY_A
		);
		if ( empty( $rows ) ) {
			return 0;
		...
		$delete = array();
		foreach ( $rows as $row ) {
			$group          = (int) $row['user_id'] . ':' . (int) $row['quiz_id'];
		...
		foreach ( array_chunk( $delete, 500 ) as $chunk ) {
			...
			$removed     += (int) $wpdb->query( $wpdb->prepare( "DELETE FROM {$table} WHERE id IN ({$placeholders})", $chunk ) );
		}
		return $removed;
	}

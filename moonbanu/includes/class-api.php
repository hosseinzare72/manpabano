...
final class MB_Api {
	...
	public static function ep_pregnancy_start( WP_REST_Request $r ) {
		...
		$raw = trim( (string) $r->get_param( 'lmp' ) );
		$lmp = MB_Jalali::parse_jalali( $raw );
		if ( ! $lmp ) {
			// sanitize_date() هر ورودی نامعتبر را «امروز» می‌کند، پس نمی‌شود آن
			// را به‌عنوان fallback بی‌قید به کار برد.
			$candidate = trim( MB_Jalali::en_num( $raw ) );
			$lmp       = MB_Jalali::is_valid( $candidate ) ? MB_Jalali::sanitize_date( $candidate ) : '';
		}
		if ( '' === (string) $lmp ) {
			MB_License::note_validation_error( 'pregnancy' );
			return new WP_Error( 'mb_pregnancy', 'تاریخ آخرین قاعدگی معتبر نیست. نمونه: ۱۴۰۴/۰۷/۰۵', array( 'status' => 400 ) );
		}

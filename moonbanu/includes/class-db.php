...
final class MB_DB {
	...
	/**
	 * رمزنگاری یادداشت خصوصی.
	 *
	 * نوشتن تازه همیشه از MB_Crypto عبور می‌کند (AES-256-GCM و در نبود GCM
	 * AES-256-CBC + HMAC-SHA256). دو ایراد نسخهٔ قبل اینجا بسته شده است:
	 *  ۱) CBC بدون احراز اصالت بود، پس متن رمز قابل دست‌کاری می‌شد.
	 *  ۲) در نبود openssl، یادداشت با پیشوند «plain:» عملاً خام ذخیره می‌شد.
	 * خواندن مقادیر قدیمی («v1:» و «plain:») همچنان کار می‌کند.
	 */
	public static function encrypt( string $plain ): string {
		if ( '' === $plain ) {
			return '';
		}
		if ( class_exists( 'MB_Crypto' ) ) {
			return MB_Crypto::encrypt( $plain );
		}
		if ( ! function_exists( 'openssl_encrypt' ) ) {
			MB_Plugin::log_security( 'crypto_unavailable', array( 'severity' => 'critical', 'scope' => 'note' ) );
			return '';
		}
		$key    = self::crypt_key();
		$iv     = random_bytes( 16 );
	...
	/** رمزگشایی، با سازگاری کامل رو به عقب: «f1:» ، «v2:» ، «v1:» و «plain:». */
	public static function decrypt( ?string $stored ): string {
		$stored = (string) $stored;
		if ( '' === $stored ) {
			return '';
		}
		// قالب تازهٔ MB_Crypto.
		if ( class_exists( 'MB_Crypto' ) && 0 === strpos( $stored, MB_Crypto::PREFIX ) ) {
			return MB_Crypto::decrypt( $stored );
		}
		...
		if ( 0 === strpos( $stored, 'v2:' ) ) {
			...
			$data = substr( $raw, 48 );
			if ( ! hash_equals( $mac, hash_hmac( 'sha256', $iv . $data, $key, true ) ) ) {
				return '';
			}
		...
		// v1 — قالب قدیمی، فقط برای خواندن داده‌های موجود.
	...
	/**
	 * قفل اتمی پاداش معرفی.
	 *
	 * یک UPDATE شرطی روی rewarded = 0؛ فقط یک درخواست برنده می‌شود، پس
	 * فعال‌سازی‌های هم‌زمان نمی‌توانند دو کوپن پاداش بسازند.
	 *
	 * @return bool true اگر همین درخواست قفل را گرفت.
	 */
	public static function claim_referral_reward( int $id ): bool {
		global $wpdb;
		$affected = $wpdb->query(
			$wpdb->prepare(
				'UPDATE ' . self::t( 'referrals' ) . ' SET rewarded = 1, rewarded_at = %s WHERE id = %d AND rewarded = 0',
				current_time( 'mysql' ),
				$id
			)
		);
	...
	/** آزادکردن قفل وقتی ساخت کوپن شکست خورد. */
	public static function release_referral_reward( int $id ): void {
		global $wpdb;
		$wpdb->query(
			$wpdb->prepare(
				'UPDATE ' . self::t( 'referrals' ) . " SET rewarded = 0, rewarded_at = NULL WHERE id = %d AND coupon = ''",
				$id
			)
		);
	}

...
final class MB_OTP {
	...
	private static function hash_code( int $user_id, string $code ): string {
		// مشتق‌سازی از هر سه نمک وردپرس به‌علاوهٔ توکن نصب: افشای یکی از آن‌ها
		// برای بازسازی هش کد کافی نیست.
		$secret = ( defined( 'AUTH_KEY' ) ? (string) AUTH_KEY : '' )
			. ( defined( 'AUTH_SALT' ) ? (string) AUTH_SALT : '' )
			. ( defined( 'NONCE_SALT' ) ? (string) NONCE_SALT : '' )
			. hash( 'sha256', 'mb_otp|' . (string) get_option( 'mb_install_token' ) );
	...
	private static function send_sms( string $mobile, string $code ) {
		...
		$password = (string) MB_Plugin::setting_secret( 'sms_api_key', '' );
	...
	/** آیا ورود با کد یک‌بارمصرف در تنظیمات روشن است؟ */
	public static function enabled(): bool {
		return 1 === (int) MB_Plugin::setting( 'otp_login', 1 );
	}

	/** خطای استاندارد وقتی مدیر این روش ورود را خاموش کرده است. */
	private static function disabled_error(): WP_Error {
		return new WP_Error( 'mb_otp_disabled', 'ورود با کد یک‌بارمصرف روی این سایت فعال نیست.', array( 'status' => 403 ) );
	}

	public static function ep_request( WP_REST_Request $r ) {
		// گیت سمت سرور: خاموش‌کردن این گزینه در پنل باید واقعاً مسیر را ببندد،
		// نه فقط پنل ورود را پنهان کند.
		if ( ! self::enabled() ) {

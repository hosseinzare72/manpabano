...
final class MB_Referral {
	...
	public static function on_pro_activated( int $referee, int $sub_id ): void {
		...
		// قفل اتمی: الگوی check-then-create بود، پس دو فعال‌سازی هم‌زمان (یا
		// تلاش دوبارهٔ کالبک درگاه) هر دو rewarded=0 می‌دیدند و دو کوپن می‌ساختند.
		if ( ! MB_DB::claim_referral_reward( (int) $row['id'] ) ) {
			return;
		}
		...
		if ( '' === $coupon ) {
			// ساخت کوپن شکست خورد: قفل آزاد شود تا تلاش بعدی ممکن بماند.
			MB_DB::release_referral_reward( (int) $row['id'] );
			return;
		}

...
final class MB_Health_Profile {
	...
	public static function save( int $user_id, array $data ): array {
		...
		// MB_Crypto::encrypt در نبود openssl یا هر خطای رمزنگاری رشتهٔ خالی
		// برمی‌گرداند. پیش از این همان خالی ذخیره و ok=true برگردانده می‌شد، یعنی
		// کد ملی و یادداشت پزشکیِ موجود بی‌صدا پاک می‌شد. حالا ذخیره رد می‌شود.
		$enc_nid  = '' === (string) $nid ? '' : MB_Crypto::encrypt( (string) $nid );
		$enc_note = '' === $note ? '' : MB_Crypto::encrypt( $note );
		if ( ( '' !== (string) $nid && '' === $enc_nid ) || ( '' !== $note && '' === $enc_note ) ) {
			MB_Plugin::log_security( 'health_encrypt_failed', array( 'severity' => 'critical', 'user' => $user_id ) );
			return array(
				'ok'        => false,
				'error'     => 'ذخیرهٔ فیلد حساس ممکن نشد (رمزنگاری در دسترس نیست). داده‌های قبلی دست‌نخورده ماند؛ با پشتیبانی تماس بگیر.',
				'bmi'       => $current['bmi'],
				'bmi_label' => '',
			);
		}

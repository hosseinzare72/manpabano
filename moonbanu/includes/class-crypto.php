...
final class MB_Crypto {
	...
	public static function decrypt( ?string $stored ): string {
		...
		if ( 0 !== strpos( $stored, self::PREFIX ) ) {
			...
			// MB_DB::decrypt مقادیر «f1:» را به همین کلاس برمی‌گرداند، پس این
			// شرط نگهبان بازگشت بی‌پایان است.
			return class_exists( 'MB_DB' ) ? MB_DB::decrypt( $stored ) : '';
		}
	...
	public static function admin_notice(): void {
		...
		echo '<div class="notice notice-warning"><p><strong>ماه‌بانو:</strong> کلید اختصاصی رمزنگاری فیلدهای حساس تعریف نشده است. '
			...
			. '<p><code>define( \'MB_FIELD_KEY\', \'' . esc_html( self::suggested_key() ) . '\' );</code></p>'
	...
	/**
	 * پیشنهاد کلید، پایدار برای چند ساعت.
	 *
	 * پیش از این هر بار بارگذاری هر صفحهٔ مدیریت یک کلید تازه می‌ساخت؛ مدیر
	 * می‌توانست کلیدی را کپی کند و پس از نوسازی صفحه کلید دیگری ببیند. این
	 * مقدار هرگز برای رمزنگاری استفاده نمی‌شود و فقط پیشنهاد نمایشی است.
	 */
	private static function suggested_key(): string {
		$key = get_transient( 'mb_field_key_hint' );
		if ( is_string( $key ) && 64 === strlen( $key ) && preg_match( '/^[0-9a-f]{64}$/', $key ) ) {
			return $key;
		}
		$key = bin2hex( random_bytes( 32 ) );
		set_transient( 'mb_field_key_hint', $key, 12 * HOUR_IN_SECONDS );
		return $key;
	}

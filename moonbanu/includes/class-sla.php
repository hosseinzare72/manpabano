...
final class MB_SLA {
	...
	public static function run_digest(): void {
		...
		// admin_alert محتوا را esc_html می‌کند، پس این جدول به‌صورت متن خام برای
		// مدیر می‌رفت. اینجا مستقیم به‌صورت HTML فرستاده می‌شود.
		$to = sanitize_email( (string) MB_Plugin::setting( 'admin_email', get_option( 'admin_email' ) ) );
		if ( is_email( $to ) ) {
			MB_Notify::mail_html( $to, $subject, MB_Notify::email_wrap( $subject, $html ) );
		}
	}
}

...
final class MB_Isolation {
	...
	public static function clean_head( string $html ): string {
		...
		$keep_own = static function ( array $m ): string {
			return false !== stripos( $m[0], 'data-mb="1"' ) ? $m[0] : '';
		};
		foreach ( $patterns as $pattern ) {
			$html = self::safe_replace( $pattern, $keep_own, $html );
		}
		// استایل‌های درون‌خطی قالب که با !important رنگ و پس‌زمینه را عوض می‌کنند.
		return self::safe_replace( '#<style[^>]*>.*?</style>#is', $keep_own, $html );
	}

	/**
	 * جایگزینی امن با regex.
	 *
	 * الگوهای «.*?» با /s روی صفحه‌های بزرگ می‌توانند به سقف backtrack پی‌سی‌آر‌ای
	 * بخورند؛ در آن حالت preg_replace_callback مقدار null برمی‌گرداند و cast به
	 * رشته، کل head یا footer را پاک می‌کرد (صفحهٔ سفید). اینجا در هر خطا متن
	 * اصلی دست‌نخورده برمی‌گردد.
	 */
	private static function safe_replace( string $pattern, callable $callback, string $html ): string {

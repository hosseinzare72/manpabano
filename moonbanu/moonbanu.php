...
final class MB_Plugin {
	...
	/**
	 * رمزنگاری یک‌بارهٔ تنظیمات حساس (idempotent).
	 *
	 * کلیدهای api پیامک و کد مرچنت درگاه‌ها پیش‌تر خام در wp_options می‌نشستند؛
	 * هر افزونه یا بک‌آپی که options را می‌خواند به آن‌ها دسترسی داشت. این متد
	 * مقدارهای legacy را با MB_Crypto رمز می‌کند و اجرای مکرر آن بی‌اثر است.
	 */
	public static function migrate_sensitive_settings(): void {
		$s = get_option( 'mb_settings', array() );
		if ( ! is_array( $s ) ) {
			return;
		}
		$changed = false;
		foreach ( MB_Admin::SENSITIVE_KEYS as $key ) {
			$val = (string) ( $s[ $key ] ?? '' );
			if ( '' === $val || 0 === strpos( $val, MB_Crypto::PREFIX ) ) {
				continue;
			}
			$enc = MB_Crypto::encrypt( $val );
			if ( '' !== $enc ) {
				$s[ $key ] = $enc;
				$changed   = true;
			}
		...
		if ( $changed ) {
			...
			self::log_security( 'settings_encrypted', array( 'keys' => MB_Admin::SENSITIVE_KEYS ) );
		}
	}
	...
	/** خواندن تنظیم حساس با پشتیبانی از مقدار legacy و مقدار رمزنگاری‌شده. */
	public static function setting_secret( string $key, $default = '' ) {
		$val = (string) self::setting( $key, $default );
		if ( '' === $val ) {
			return $default;
		}
		if ( 0 === strpos( $val, MB_Crypto::PREFIX ) ) {
			$dec = MB_Crypto::decrypt( $val );
			return '' !== $dec ? $dec : $default;
		}
		return $val;
	}
